// ============================================================================
// commands/2x2mob.js - IGUAL AO 1x1mob (só muda título e botões)
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[2x2mob] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[2x2mob] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`2x2 Mobile | R$ ${formatMoney(valor)}`)
    ...
    collector.on("collect", async interaction => {
      ...
      if (id === `entrar_${valor}`) {
        ...
        if (!fila.jogador) {
          ...
          const j1 = fila.jogador;
          const j2 = userId;
          fila.jogador = null;

          const mediador = escolherMediador(mediadores);
          await criarThread(msg, j1, j2, mediador, valor, client);
        }
      }
      ...
      const vJogador = fila.jogador ? `<@${fila.jogador}>` : "Nenhum jogador na fila.";

      const newEmbed = new EmbedBuilder()
        .setTitle(`2x2 Mobile | R$ ${formatMoney(valor)}`)
        .setColor(CONFIG.CORES.FILA)
        .addFields(
          { name: "Jogadores:", value: vJogador }
        );
...
async function criarThread(msg, j1, j2, mediador, valor, client) {
  let apostasId;
  try {
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, j1, j2, mediadorFinal, "2x2mob", valor, []);
  ...
  collector.on("collect", async i => {
    ...
    if (i.customId === "confirmar") {
      ...
      if (confirmados.has(j1) && confirmados.has(j2)) {
        ...
        deleteCollector(confirmMsg.id);
        await aguardarSala(thread, j1, j2, mediadorFinal, valor);
      }
    }
  });
...
export async function aguardarSala(thread, j1, j2, mediador, valor) {
  ...
  msgCollector.on("collect", async m => {
    ...
    registrarFilaMediada(mediador.id, mediador.nome, "2x2mob", thread.id);
