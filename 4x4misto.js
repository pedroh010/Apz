// ============================================================================
// commands/4x4misto.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[4x4misto] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[4x4misto] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
    ...
    for (const valor of CONFIG.VALORES) {
      ...
      const row = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
          ...
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
          .setCustomId(`3emu_${valor}`)
          .setLabel("3 Emuladores")
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
  ...
  startCollector(msg, client, valor, webhook) {
    ...
    collector.on("collect", async interaction => {
      ...
      if (id === `3emu_${valor}`) {
        ...
        if (fila.jogadores3emu.includes(userId)) {
          delete fila[`_lock_${lockKey}`];
          return interaction.followUp({
            content: "Você já está na fila de 3 Emulador!",
            flags: MessageFlags.Ephemeral
          }).catch(() => {});
        }
        ...
        if (fila.jogadores3emu.length < 2) {
          if (!canJoinQueue(userId)) {
            delete fila[`_lock_${lockKey}`];
            return interaction.followUp({
              content: "Você já está em três filas simultâneas.",
          ...
          fila.jogadores3emu.push(userId);
        }

        if (fila.jogadores3emu.length === 2) {
          const jogadores = [...fila.jogadores3emu];
          fila.jogadores3emu = [];

          const mediador = escolherMediador(mediadores);
          await criarThread(msg, jogadores, mediador, valor, client, "3emu");
        }
      }
      ...
      if (id === `sair_${valor}`) {
        ...
        const index3 = fila.jogadores3emu.indexOf(userId);
        ...
        if (index3 !== -1) fila.jogadores3emu.splice(index3, 1);
      }
      ...
      const vJogadores3emu = fila.jogadores3emu.length === 0 
        ? "Nenhum jogador na fila." 
        : fila.jogadores3emu.map(id => `<@${id}>`).join("\n");
      ...
      const newEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "3 Emuladores:", value: vJogadores3emu }
        );
...
async function criarThread(msg, jogadores, mediador, valor, client, tipoFila) {
  ...
  const tipoTexto = tipoFila === "1emu" ? "1 Emulador" : tipoFila === "2emu" ? "2 Emulador" : "3 Emulador";
  
  const embed = new EmbedBuilder()
    .setTitle(`4x4 Misto - ${tipoTexto} | R$ ${formatMoney(valor)}`)
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "4x4misto", valor, jogadores);
