// ============================================================================
// commands/4x4mob.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[4x4mob] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[4x4mob] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`4x4 Mobile | R$ ${formatMoney(valor)}`)
...
async function criarThread(msg, jogadores, mediador, valor, client) {
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "4x4mob", valor, jogadores);
...
export async function aguardarSala(thread, jogadores, mediador, valor) {
  ...
  msgCollector.on("collect", async m => {
    ...
    registrarFilaMediada(mediador.id, mediador.nome, "4x4mob", thread.id);
