// ============================================================================
// commands/3x3mob.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[3x3mob] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[3x3mob] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`3x3 Mobile | R$ ${formatMoney(valor)}`)
...
async function criarThread(msg, jogadores, mediador, valor, client) {
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "3x3mob", valor, jogadores);
...
export async function aguardarSala(thread, jogadores, mediador, valor) {
  ...
  msgCollector.on("collect", async m => {
    ...
    registrarFilaMediada(mediador.id, mediador.nome, "3x3mob", thread.id);
