// ============================================================================
// commands/1x1emu.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[1x1emu] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[1x1emu] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`1x1 Emulador | R$ ${formatMoney(valor)}`)
...
async function criarThread(msg, jogadores, mediador, valor, client) {
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "1x1emu", valor, jogadores);
...
export async function aguardarSala(thread, jogadores, mediador, valor) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Formato", value: "1x1 Emulador" },
