// ============================================================================
// commands/2x2emu.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[2x2emu] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[2x2emu] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`2x2 Emulador | R$ ${formatMoney(valor)}`)
  ...
  startCollector(msg, client, valor, webhook) {
    ...
    collector.on("collect", async interaction => {
      ...
      const newEmbed = new EmbedBuilder()
        ...
        .setColor(CONFIG.CORES.FILA)
        .addFields(
          { name: "Jogadores:", value: vJogadores }
        );
...
async function criarThread(msg, jogadores, mediador, valor, client) {
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "2x2emu", valor, jogadores);
...
export async function aguardarSala(thread, jogadores, mediador, valor) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Formato", value: "2x2 Emulador" },
