// ============================================================================
// commands/2x2misto.js - COM WEBHOOKS AUTOMÁTICOS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[2x2misto] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[2x2misto] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`2x2 Misto | R$ ${formatMoney(valor)}`)
    ...
    collector.on("collect", async interaction => {
      ...
      if (id === `1emu_${valor}`) {
        ...
        if (fila.jogadores.length === 2) {
          ...
          await criarThread(msg, jogadores, mediador, valor, client);
        }
      }

      if (id === `sair_${valor}`) {
        const index = fila.jogadores.indexOf(userId);
        if (index === -1) {
          delete fila[`_lock_${lockKey}`];
          return interaction.followUp({
      ...
      const vJogadores = fila.jogadores.length === 0 
      ...
      const newEmbed = new EmbedBuilder()
        ...
        .setColor(CONFIG.CORES.FILA)
        .addFields(
          { name: "1 Emulador:", value: vJogadores }
        );
...
async function criarThread(msg, jogadores, mediador, valor, client) {
  let apostasId;
  try {
  ...
  const embed = new EmbedBuilder()
    .setTitle(`2x2 Misto - 1 Emulador | R$ ${formatMoney(valor)}`)
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, jogadores[0], jogadores[1], mediadorFinal, "2x2misto", valor, jogadores);
  ...
  collector.on("collect", async i => {
    ...
    if (i.customId === "confirmar") {
      ...
      if (jogadores.every(j => confirmados.has(j))) {
        ...
        deleteCollector(confirmMsg.id);
        await aguardarSala(thread, jogadores, mediadorFinal, valor);
      }
    }
  });
...
export async function aguardarSala(thread, jogadores, mediador, valor) {
