// ============================================================================
// commands/1x1mob.js - COM FATURAMENTO INTEGRADO E WEBHOOKS
// ============================================================================
...
function saveThreadCounter(counter) {
  try {
    ...
    console.error("[1x1mob] Erro ao salvar contador:", err.message);
  }
}
...
async function createWebhookForQueue(channel, valor) {
  try {
    ...
    console.error(`[1x1mob] Erro ao criar webhook para R$ ${formatMoney(valor)}:`, err.message);
    return null;
  }
}
...
export default {
  ...
  async execute(message) {
    for (const valor of CONFIG.VALORES) {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`1x1 Mobile | R$ ${formatMoney(valor)}`)
        .setColor(CONFIG.CORES.FILA)
        .addFields(
          { name: "Gel Normal:", value: "Nenhum jogador na fila." },
          { name: "Gel Inf:", value: "Nenhum jogador na fila." }
        );

      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`gelNormal_${valor}`)
          .setLabel("Gel Normal")
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
          .setCustomId(`gelInf_${valor}`)
          .setLabel("Gel Inf")
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
  ...
  startCollector(msg, client, valor, webhook) {
    const fila = { gelNormal: null, gelInf: null };
    ...
    collector.on("collect", async interaction => {
      ...
      if (id === `gelNormal_${valor}` || id === `gelInf_${valor}`) {
        const tipo = id.includes("gelNormal") ? "gelNormal" : "gelInf";
        ...
        if (!fila[tipo]) {
          ...
          await criarThread(msg, j1, j2, mediador, tipo, valor, client);
        }
      }

      if (id === `sair_${valor}`) {
        if (fila.gelNormal === userId) fila.gelNormal = null;
        else if (fila.gelInf === userId) fila.gelInf = null;
        else {
          delete fila[`_lock_${lockKey}`];
          return interaction.followUp({
      ...
      const vNormal = fila.gelNormal ? `<@${fila.gelNormal}>` : "Nenhum jogador na fila.";
      const vInf = fila.gelInf ? `<@${fila.gelInf}>` : "Nenhum jogador na fila.";

      const newEmbed = new EmbedBuilder()
        .setTitle(`1x1 Mobile | R$ ${formatMoney(valor)}`)
        .setColor(CONFIG.CORES.FILA)
        .addFields(
          { name: "Gel Normal:", value: vNormal },
          { name: "Gel Inf:", value: vInf }
        );
...
async function criarThread(msg, j1, j2, mediador, tipo, valor, client) {
  let apostasId;
  try {
  ...
  const threadName = tipo === "gelInf"
    ? `fila-${idSequencial}-inf`
    : `fila-${idSequencial}`;
  ...
  registerThread(thread.id, [j1, j2]);
  thread.jogadores = [j1, j2];
  ...
  const tipoTexto = tipo === "gelNormal" ? "Gel Normal" : "Gel Inf";

  await thread.send(`<@${j1}> <@${j2}>`);

  const embed = new EmbedBuilder()
    .setTitle(`1x1 Mobile - ${tipoTexto} | R$ ${formatMoney(valor)}`)
  ...
  saveThreadConfirmacao(thread.id, confirmMsg.id, thread.id, j1, j2, mediadorFinal, tipo, valor, []);
  ...
  collector.on("collect", async i => {
    ...
    if (i.customId === "confirmar") {
      ...
      if (confirmados.has(j1) && confirmados.has(j2)) {
        ...
        deleteCollector(confirmMsg.id);
        await aguardarSala(thread, j1, j2, mediadorFinal, valor, tipoTexto);
      }
    }
  });
...
// ============================================================================
// AGUARDAR SALA - EXPORTAR PARA USAR NO COLLECTOR MANAGER
// ============================================================================

export async function aguardarSala(thread, j1, j2, mediador, valor, tipoTexto) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "Jogadores:", value: `<@${j1}> vs <@${j2}>` },
      { name: "Mediador:", value: `<@${mediador.id}>` },
      { name: "Formato", value: "1x1 Mobile" },
  ...
  msgCollector.on("collect", async m => {
    ...
    await operario.monitorarThread(thread, [j1, j2]);
